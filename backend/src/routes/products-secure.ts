...
router.get('/', 
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    let filteredProducts = products.filter(product => {
      ...
      if (search && !product.name.toLowerCase().includes(search.toString().toLowerCase())) return false;
...
router.get('/:id',
  validate([validationGroups.product.update[0]]), // Validate ID parameter
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.READ),
  requireOwnership((req: AuthRequest) => {
    const product = products.find(p => p.id === req.params.id);
    return product?.ownerId || '';
  }),
  auditLog('product_view', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.post('/',
  validate(validationGroups.product.create),
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.WRITE),
  auditLog('product_create', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user?.id;
    const {
      name,
...
// Update product
router.put('/:id',
  validate([...validationGroups.product.update, ...validationGroups.product.create.slice(1)]),
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.WRITE),
  requireOwnership((req: AuthRequest) => {
    const product = products.find(p => p.id === req.params.id);
  ...
  auditLog('product_update', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.delete('/:id',
  validate([validationGroups.product.update[0]]),
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.DELETE),
  requireOwnership((req: AuthRequest) => {
    const product = products.find(p => p.id === req.params.id);
    return product?.ownerId || '';
  }),
  auditLog('product_delete', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
...
router.post('/:id/checkpoints',
  validate([validationGroups.checkpoint.create[0], ...validationGroups.checkpoint.create.slice(1)]),
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.WRITE),
  requireOwnership((req: AuthRequest) => {
    const product = products.find(p => p.id === req.params.id);
  ...
  auditLog('checkpoint_create', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.get('/:id/checkpoints',
  ...
  auditLog('checkpoint_list', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
...
router.post('/:id/verify',
  validate([validationGroups.product.update[0]]),
  sanitize,
  requireResourcePermission(Resource.PRODUCT, Permission.VERIFY),
  auditLog('product_verify', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.get('/stats/overview',
  requireResourcePermission(Resource.PRODUCT, Permission.READ),
  auditLog('product_stats', Resource.PRODUCT),
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const userId = req.user?.id;
    ...
    const stats = {
      ...
      totalCheckpoints: checkpoints.filter(cp => 
        userProducts.some(p => p.id === cp.productId)
      ).length,
