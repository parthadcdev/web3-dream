...
const requireRole = (roles) => {
    return (req, res, next) => {
        if (!req.user) {
            next((0, errorHandler_js_1.createError)('Authentication required', 401));
            return;
        }
        if (!roles.includes(req.user.role)) {
            next((0, errorHandler_js_1.createError)('Insufficient permissions', 403));
            return;
        }
        next();
    };
...
const requireWallet = (req, res, next) => {
    if (!req.user?.walletAddress) {
        next((0, errorHandler_js_1.createError)('Wallet address required', 400));
        return;
    }
    next();
};
