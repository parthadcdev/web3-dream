...
describe("ProductRegistry CRUD Operations", function () {
    let productRegistry;
    ...
    describe("Checkpoint CRUD Operations", function () {
        beforeEach(async function () {
            // Register a test product
            await productRegistry.connect(manufacturer).registerProduct(
                "Checkpoint Test",
        ...
        describe("Checkpoint Creation", function () {
        ...
        describe("Checkpoint Retrieval", function () {
            ...
            it("Should retrieve all checkpoints", async function () {
                const checkpoints = await productRegistry.getCheckpoints(1);
        ...
        describe("Checkpoint Updates", function () {
            beforeEach(async function () {
                await productRegistry.connect(manufacturer).addCheckpoint(
                    1, "manufactured", "Facility A", "Original data"
                );
            });

            it("Should update checkpoint additional data", async function () {
                await productRegistry.connect(manufacturer).updateCheckpointData(
                    1, 1, "Updated additional data"
                );

                const checkpoints = await productRegistry.getCheckpoints(1);
                expect(checkpoints[1].additionalData).to.equal("Updated additional data");
            });

            it("Should prevent unauthorized checkpoint updates", async function () {
                await expect(
                    productRegistry.connect(retailer).updateCheckpointData(
                        1, 1, "Unauthorized update"
                    )
                ).to.be.revertedWith("Not authorized stakeholder");
            });
        });
    ...
    describe("Edge Cases and Error Handling", function () {
        it("Should handle zero address manufacturer", async function () {
            await expect(
                productRegistry.connect(manufacturer).registerProduct(
                    "Test Product", "pharmaceutical", "ZERO-001",
                    Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000) + 86400 * 365,
                    ["Material"], "https://ipfs.io/hash"
                )
            ).to.not.be.reverted; // This should work as msg.sender is used
        });
