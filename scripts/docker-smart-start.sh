...
start_services() {
    ...
    docker-compose -f $compose_file --profile all up -d $services_to_start
...
show_logs() {
    ...
    if [ -n "$service" ]; then
        docker-compose -f docker-compose.smart.yml logs -f $service
    else
...
show_help() {
    echo "Smart Docker Compose Management"
    echo "==============================="
    echo ""
