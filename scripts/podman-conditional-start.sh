...
# Conditional Podman Compose Startup Script
# This script uses advanced Podman Compose features to conditionally start services
...
start_conditional_services() {
    ...
    if [ -n "$COMPOSE_PROFILES" ]; then
        ...
        COMPOSE_PROFILES=$COMPOSE_PROFILES podman-compose -f $compose_file up -d
    else
...
show_conditional_status() {
    ...
    elif podman ps --format "table {{.Names}}" | grep -q "tracechain-postgres"; then
    ...
    elif podman ps --format "table {{.Names}}" | grep -q "tracechain-redis"; then
...
show_help() {
    echo "Conditional Podman Compose Management"
    echo "====================================="
    echo ""
    ...
    echo "  • Uses appropriate Podman Compose configuration"
